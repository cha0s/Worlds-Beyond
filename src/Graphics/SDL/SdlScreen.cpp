...
void SdlScreen::set(int width, int height, ScreenFlags f) {
	...
	if (SDL_getenv(const_cast<char *>("WB_SDL_VIDEORESTORE"))) {
		SDL_QuitSubSystem(SDL_INIT_VIDEO);

		std::string *driver = new std::string("SDL_VIDEODRIVER=");

		const char *currentDriver = SDL_getenv(const_cast<char *>("WB_SDL_VIDEODRIVER"));
...
#ifdef __MINGW32__
		SDL_putenv(const_cast<char *>("WB_SDL_VIDEORESTORE="));
#else
		unsetenv("WB_SDL_VIDEORESTORE");
#endif
